    ...
    def net(self, input, is_training = True, reuse = False, scope = 'Generator'):
        with tf.variable_scope(scope, 'Generator', [input], reuse = reuse ) as net_scope:
            with slim.arg_scope(model_arg_scope(is_training = is_training)):
                ...
                # b* 4*4 *
                net = slim.conv2d_transpose(net, self.ngf*8, [4,4], stride = 1, scope = 'g_deconv_1')
                # b* 8*8 *
                ...
                # b* 32*32 *
                net = slim.conv2d_transpose(net, self.ngf, [4,4], stride = 2 , scope = 'g_deconv_4')
    ...
    def net(self, input, is_training = True, reuse = False, scope = 'Encoder'):
        with tf.variable_scope(scope, 'Encoder', [input], reuse = reuse ) as net_scope:
            with slim.arg_scope(model_arg_scope(is_training = is_training, ac_fn = _leaky_relu)):
                if reuse:
                    net_scope.reuse_variables()
                net = slim.conv2d(input, self.ndf, [4,4], stride = 2, normalizer_fn = None, scope = 'e_conv_1')
                net = slim.conv2d(net, self.ndf*2, [4,4], stride = 2, scope = 'e_conv_2')
                net = slim.conv2d(net, self.ndf*4, [4,4], stride = 2, scope = 'e_conv_3')
                net = slim.conv2d(net, self.nz, [4,4], stride = 2, normalizer_fn = None, activation_fn = None, scope = 'e_conv_4')
