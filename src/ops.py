...
def make_z(flags):
    z = tf.random_normal([flags.batch_size, flags.nz, 1,1])
...
def normalize(x, dim = 1):
    # project into sphere
    return tf.div(x,tf.expand_dims(tf.norm(x,axis = 1),1))
...
def KL_Gaussian(samples, direction,is_minimize = True):
    ...
    if direction == 'pq':
        # mu_1 = 0; sigma_1 = 1
        term1 = (1+ tf.pow(samples_mean,2)) / (2 * tf.pow(samples_var))
        term2 = tf.log(samples_var)
        KL = tf.reduce_mean(term1 + term2 -0.5)
    else :
        # mu_2 = 0; sigma_2 = 1
        term1 = (tf.pow(samples_mean,2) + tf.pow(samples_var)) / 2
        term2 = -tf.log(samples_var)
        KL = tf.reduce_mean(term1 + term2 - 0.5)
